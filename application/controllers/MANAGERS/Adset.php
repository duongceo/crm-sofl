...
//        $data['top_nav'] = 'manager/common/top-nav';
        ...
        $data['list_title'] = 'Adset (tính theo giờ Mỹ)';
    ...
    function action_add_item() {
        ...
        if (!empty($post)) {
            ...
            $input['select'] = 'channel_id, account_fb_id';

            $input['where'] = array('id' => $param['campaign_id']);

            $cam = $this->campaign_model->load_all($input);

            if (!empty($cam)) {

                $param['channel_id'] = $cam[0]['channel_id'];
                $param['account_fb_id'] = $cam[0]['account_fb_id'];
    ...
    function action_edit_item($id) {
        ...
        if (!empty($post)) {
			...
			$input = array();
            $input['select'] = 'adset_id_facebook';
            $input['where']['id'] = $id;
            $adset_id_facebook = $this->{$this->model}->load_all($input);
            
            if ($post['edit_adset_id_facebook'] != $adset_id_facebook[0]['adset_id_facebook'] && $this->{$this->model}->check_exists(array('adset_id_facebook' => $post['edit_adset_id_facebook']))) {
            ...
            $input2['select'] = 'channel_id, account_fb_id';

            $input2['where'] = array('id' => $param['campaign_id']);

            $cam = $this->campaign_model->load_all($input2);
    ...
    public function AddItemFetch() {
        ...
        foreach ($campaigns as $key => $value2) {
            ...
            if ($value2['campaign_id_facebook'] != '') {
                ...
                $url = 'https://graph.facebook.com/v3.1/' . $value2['campaign_id_facebook'] . '/' .
