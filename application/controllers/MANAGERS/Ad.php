...
//    }
    ...
    function index($offset = 0) {
        ...
        $data['list_title'] = 'Ads (tính theo giờ Mỹ)';
    ...
    function action_add_item() {
        ...
        if (!empty($post)) {
            ...
            if (!empty($adset)) {
                ...
                $param['channel_id'] = $adset[0]['channel_id'];
                $param['account_fb_id'] = $adset[0]['account_fb_id'];
...
//            print_arr($adset);
    ...
    function action_edit_item($id) {
        ...
        if (!empty($post)) {
			...
			$input = array();
            $input['select'] = 'ad_id_facebook';
            $input['where']['id'] = $id;
            $ad_id_facebook = $this->{$this->model}->load_all($input);
            
            if ($post['edit_ad_id_facebook'] != $ad_id_facebook[0]['ad_id_facebook'] && $this->{$this->model}->check_exists(array('ad_id_facebook' => $post['edit_ad_id_facebook']))) {
            ...
            $adset = $this->adset_model->load_all($input2);
