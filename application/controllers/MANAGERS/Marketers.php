...
//        }
    ...
    function action_edit_item($id) {
        ...
        if (!empty($post)) {
            ...
            $paramArr = array('username', 'name', 'email', 'phone', 'active','warehouse', 'targets', 'lever');            foreach ($paramArr as $value) {                if (isset($post['edit_' . $value])) {                    if ($value == 'warehouse') {                        $param[$value] = implode(',', $post['edit_' . $value]);                    } else {                        $param[$value] = $post['edit_' . $value];                    }                }else{					if ($value == 'warehouse') {                        $param[$value] = '';                    }				}            }			$param['short_name'] = $post['edit_username'];			
