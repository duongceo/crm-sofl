...
class Check_L8_vnpost extends MY_Table {
    ...
    public function init() {
        ...
        $this->controller_path = 'CODS/check_L8_vnpost';

        $this->view_path = 'cod/check_L8_vnpost';
    ...
    function index($offset = 0) {
        ...
        $data['slide_menu'] = 'cod/check_L8_vnpost/slide-menu';

        $data['top_nav'] = 'manager/common/top-nav';

        $data['list_title'] = 'Kết quả đối soát L8 VNPOST';

        $data['edit_title'] = 'Sửa thông tin dòng đối soát L8';

        $data['content'] = 'cod/check_L8_vnpost/index';
    ...
    public function upload_file() {
        ...
        if (!empty($_FILES)) {
			...
			//echo '<pre>';print_r($targetFile);die();
            ...
            $data['content'] = 'cod/check_L8_vnpost/upload';
...
//        }
    ...
    private function _import_L8($file_path) {
        ...
        $data1 = $sheet->rangeToArray('A1:F7000');
         ...
         * Mảng lưu các thông tin contact L8

         * Cần lưu hết thông tin để check trùng (đề phòng trường hợp người dùng tải trùng file)
        ...
        foreach ($data1 as $row) {
            ...
            list($stt, $code_cross_check, $money, $name, $address, $phone) = $row;
...
//				$phone = intval($phone);
				...
				$receiveCOD[] = array(
					...
					'date_deliver_success' => date('d/m/Y'),
...
//		echo '<pre>';print_r($receiveCOD);die();
        ...
        foreach ($receiveCOD as $key => $value) {
            ...
            // $c_id = $this->h_find_contact($value['phone']);

			//echo '<pre>';print_r($c_id);
...
//                $receiveCOD[$key]['address'] = $contact[0]['address'];
            ...
            } else { //Nếu không thì bỏ trống thông tin khách hàng, đồng thời báo đỏ (is_match = 0)
        ...
        redirect(base_url('cod/doi-soat-l8-vnpost.html'));
    ...
    function confirm_check_l8() {
        ...
        foreach ($post['item_id'] as $value) {
            ...
            $code_cross = $this->{$this->model}->load_all($input);
			//echo '<pre>';print_r($code_cross);die();
			...
			// $input['where'] = array('phone' => $code_cross[0]['phone']);
            ...
            $data = array(
            	// 'code_cross_check' => $code_cross[0]['code'],

            	'date_receive_lakita' => time(),
...
// 	private function h_find_contact($phone) {
// 		$input = array();
// 		$input['select'] = 'id';
...
// 	}
    ...
    private function _import_L8_old($file_path) {
     ...
     public function update_code_check() {
...
//      echo '<pre>';print_r($_FILES);die();
        ...
        if (!empty($_FILES)) {
            ...
            $targetFile = APPPATH . '../public/upload/update_code_vnpost/' . date('Y-m-d-H-i') . '.' . pathinfo($fileName, PATHINFO_EXTENSION);
            ...
            $data = $sheet->rangeToArray('A1:D500');
...
//              $input['where'] = array('phone' => $value[2], 'code_cross_check' => $value[0]);
//              $contact = $this->contacts_model->load_all($input);
//              echo '<pre>'; print_r($contact); echo '<br>';
                $where = array('phone' => $value[2], 'code_cross_check' => $value[0]);
                $data = array('code_cross_check' => $value[1]);

                $this->contacts_model->update($where, $data);
            }

            redirect(base_url('cod/contact-dang-giao-hang.html'));
