...
//		}
	...
	public function view_report_quality_contact() {
		...
		if (isset($get['tic_report'])) {
			$typeReport = array(
				'C3' => array(
					'where' => array('is_old' => '0', 'call_status_id NOT IN (1, 3)' => 'NO-VALUE', 'level_contact_detail NOT IN ("L1.1", "L1.2")' => 'NO-VALUE', 'source_id IN (1, 2, 6, 8)' => 'NO-VALUE', 'date_rgt >=' => $date_from, 'date_rgt <=' => $date_end),
				),
				'L5' => array(
...
//			$date_end = strtotime(str_replace("/", "-", $date_end)) + 3600 * 24 - 1;
		...
		$typeReport = array(
			'L1' => array(
				'where' => array('is_old' => '0', 'duplicate_id' => '0', 'source_id IN (1, 2, 6, 8)' => 'NO-VALUE', 'call_status_id NOT IN (1, 3)' => 'NO-VALUE', 'level_contact_detail NOT IN ("L1.1", "L1.2")' => 'NO-VALUE'),
